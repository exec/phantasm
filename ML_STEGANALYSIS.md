...
## Update 1 — UERD fine-tune (Option B complete)
...
### Option B' — Validate the asymmetry before committing to Option C

**The case:** The Option B finding hinges on a directional asymmetry: UERD-aware fine-tune lifts UERD detection but slightly hurts J-UNIWARD detection. If the reverse direction (J-UNIWARD-aware fine-tune from a UERD-trained baseline) also lifts J-UNIWARD detection symmetrically, the asymmetry is fake and Option B's framing is wrong. **This needs verification before committing to Option C's 2+ week scope.**

**Concrete tasks:**
1. Train a UERD-aware baseline from scratch (or fine-tune JIN-SRNet on UERD pairs aggressively, ~5 minutes).
2. From that baseline, fine-tune on J-UNIWARD pairs and measure J-UNIWARD detection lift.
3. Repeat with bigger training data: extend the Picsum corpus to 500+ images (cheap fetch) or download a BOSSbase subset.
4. Cross-passphrase: train and eval with multiple passphrases per cover to factor out passphrase-specific STC permutation artifacts.
5. If asymmetry holds → publish, then commit to Option C.
6. If asymmetry collapses → Option C needs a different framing.

**Cost:** ~half-day of work, mostly mechanical.

### Option C — Adversarial cost functions

**The case:** Most ambitious. Use the JIN-SRNet checkpoint as a differentiable distortion oracle and compute embedding costs that maximize the distance from the detector's stego decision boundary. Holub & Fridrich 2014 (J-UNIWARD) was hand-designed; modern (2019+) work uses GAN/adversarial training to learn costs that beat specific detectors. Phantasm could ship a `--cost-function adv-jin-srnet` that does this.

**Risks:** Substantial implementation work. The PyTorch graph would need to be wired into phantasm-cost (which is currently a Rust crate, no GPU). Easiest path: a Python pre-pass that produces a per-coefficient cost map, written to a sidecar file, then read by the existing Rust embed pipeline. Significant scope creep but research-grade payoff.

**What this enables:** A genuinely novel result — a publishable claim that phantasm's adv-cost mode beats SRNet at payloads where hand-designed costs don't.

### Recommended path (updated)

**Option A** (cheap docs + threat-model framing in CLI help) — **DONE.**
**Option B** (UERD fine-tune experiment) — **DONE, see § Update 1.**
**Option B'** (validate the asymmetry direction) — **next.** Cheap, ~half-day, produces decision evidence for Option C.
**Option C** (adversarial costs) — gated on Option B' result. Commit only if B' confirms the asymmetry is real.

Total scope to v0.2 release: ~2 weeks of focused work, 1 week of polish, conditional on B' result.
