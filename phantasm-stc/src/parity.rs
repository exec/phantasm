// H̃ sub-matrix tables for Syndrome-Trellis Codes.
//
// Source: the DDE Lab C++ reference implementation accompanying
//
//     T. Filler, J. Judas, J. Fridrich,
//     "Minimizing Additive Distortion in Steganography Using
//      Syndrome-Trellis Codes",
//     IEEE Transactions on Information Forensics and Security,
//     vol. 6, no. 3, pp. 920–935, September 2011.
//
// The tables below are transcribed verbatim from `common.cpp` (the
// `u32 mats[]` array) of the reference package `ml_stc_linux_make_v1.0`,
// available at http://dde.binghamton.edu/download/syndrome/ .
// These are the same H̃ columns used by the authors to report
// rate-distortion numbers in the paper, obtained by search over
// random submatrices constrained so that the top (bit h-1) and
// bottom (bit 0) rows are always 1 (see common.cpp line 152ff).
//
// Layout of DDE_MATS:
//   index = (h - 7) * 400 + (w - 1) * 20 + j
//   h ∈ [7, 12]  (constraint height)
//   w ∈ [1, 20]  (inverse rate / block width)
//   j ∈ [0, w)   (column index within H̃)
// Each stored value is a column of H̃ as a bitmask; bit k ↔ H̃[k][j],
// i.e. the least significant bit is row 0.
//
// For constraint heights or widths outside [7..12] × [1..20] we fall
// back to a deterministic PRNG construction (SplitMix64 + GF(2) rank
// repair). The PRNG is only reached for research configurations the
// paper did not tabulate; production phantasm presets always hit the
// tabulated range.
...
/// Generate column j of H̃ for given h_eff using SplitMix64.
/// Used as a deterministic fallback when (h, w) falls outside the
/// published DDE Lab table range.
#[inline]
...
fn prng_htilde(h: usize, w: usize) -> Vec<u64> {
    let mask = if h == 64 { u64::MAX } else { (1u64 << h) - 1 };
    // Emulate the DDE Lab construction: force bit 0 and bit h-1 of every
    // column (see common.cpp line 152ff — `bop = (1 << (h-1)) + 1`).
    // This guarantees every column is non-zero and the matrix has
    // non-trivial coupling between the top and bottom of the shift window.
    let forced_bits = if h >= 2 {
        (1u64 << (h - 1)) | 1u64
    } else {
        1u64
    };
    let inner_mask = mask & !forced_bits;

    let mut cols: Vec<u64> = (0..w)
        .map(|j| {
            let raw = prng_column(j, h);
            (raw & inner_mask) | forced_bits
        })
        .collect();

    // Target rank: min(h, w). When w < h the matrix can be rank-deficient
    // (only w controllable syndrome bits) — that is the paper's operating
    // regime and the Viterbi handles it correctly.
    let target_rank = h.min(w);
    let mut row_basis: Vec<(usize, u64)> = Vec::with_capacity(target_rank);
    for col_val in cols.iter().take(w) {
        if row_basis.len() == target_rank {
            break;
        }
        let mut v = *col_val;
    ...
    if row_basis.len() < target_rank {
        // Rare: inject standard basis vectors into spare columns until
        // we reach the target rank. Only happens for pathological PRNG
        // collisions.
        let covered: std::collections::HashSet<usize> =
            row_basis.iter().map(|&(pb, _)| pb).collect();
        let mut target = 0usize;
        for r in 0..h {
            if covered.contains(&r) || target >= w {
                continue;
            }
            cols[target] |= (1u64 << r) & mask;
            target += 1;
            if row_basis.len() + target >= target_rank {
                break;
            }
        }
    }
...
/// Look up the published H̃ columns for (h, w) from the DDE Lab tables.
/// Returns `None` if (h, w) is outside the tabulated range.
fn dde_lookup(h: u8, w: usize) -> Option<Vec<u64>> {
    if !(7..=12).contains(&h) || !(2..=20).contains(&w) {
        return None;
    }
    let base = (h as usize - 7) * 400 + (w - 1) * 20;
    let cols: Vec<u64> = DDE_MATS[base..base + w].to_vec();
    // Sanity: published columns are non-zero with bit 0 and bit (h-1) set.
    debug_assert!(cols.iter().all(|&c| c != 0));
    Some(cols)
}

/// Generate the h × w H̃ sub-matrix for STC embedding.
///
/// For h ∈ [7, 12] and w ∈ [2, 20] this returns the DDE Lab published
/// columns verbatim from Filler/Judas/Fridrich 2011. Outside that range,
/// falls back to a deterministic SplitMix64 PRNG construction with the
/// same row-forcing convention as the reference code.
///
...
pub fn htilde_for_rate(h: u8, w: usize) -> Vec<u64> {
    ...
    if let Some(cols) = dde_lookup(h, w) {
        return cols;
    }

    prng_htilde(h as usize, w)
}

/// Effective constraint height for a given configured h and inverse rate w.
///
/// Phantasm always operates the Viterbi trellis with the full configured
/// constraint height. When w < h the H̃ matrix is rank-deficient (rank
/// at most w) but the trellis is still valid: the Viterbi DP finds the
/// shortest feasible path within the constrained syndrome space.
pub fn effective_height(h: u8, _w: usize) -> usize {
    h as usize
}

// ──────────────────────────────────────────────────────────────────────────
// DDE Lab published H̃ table (verbatim transcription of common.cpp `mats[]`)
// ──────────────────────────────────────────────────────────────────────────

/// Flat array of 2400 H̃ submatrix column values, indexed as
///   DDE_MATS[(h - 7) * 400 + (w - 1) * 20 + j]
/// where h ∈ [7, 12], w ∈ [1, 20], j ∈ [0, w).
/// Width w=1 is unused (zero-padded). Values beyond index j ≥ w are 0.
#[rustfmt::skip]
static DDE_MATS: [u64; 2400] = [
