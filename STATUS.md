...
**Date:** 2026-04-25 (v0.4.0 shipped)
**Session length:** ~13 hours v0.1.0 + ML-steganalysis research drive for v0.2.0 + L1/channel-adapter/PNG burst for v0.3.0 + lossy-channel diagnostic tooling and L1-defense research arc for v0.4.0
**Workspace state:** `cargo test --workspace` **280 passing / 0 failing**, `cargo clippy` clean, `cargo fmt` clean
**Git state:** `v0.1.0-alpha` at commit `82b89a4`, `v0.1.0` at commit `1617dad`, `v0.2.0` at commit `8432cf7`, `v0.3.0` at commit `e9b3d03`, **`v0.4.0` at the release commit of this update**. Pushed to `https://github.com/exec/phantasm`.
...
**v0.4.0 shipping headline:** Plumbing release for the lossy-channel rewrite arc + a wet-paper sidecar correctness fix. (1) **Lossy-channel diagnostic tooling** lands (commit `7116571`): three runtime env overrides (`PHANTASM_LOSSY_ECC_{DATA,PARITY,SHARD}`, `PHANTASM_SALT_QUANT_STEP`), a `pipeline::diagnostics` module exposing `salt_of_jpeg` / `phash_block_of_jpeg` / `embed_capture_pre_stc` / `extract_raw_stc`, and five new `phantasm-bench` probes (`post-stc-probe`, `salt-drift-probe`, `salt-magnitude-probe`, `coeff-parity-probe`, `stego-parity-probe`). Pre-shipping measurement surfaced a v0.3 correctness bug: `SALT_QUANT_STEP=16` silently drifts on **42.5%** of covers through `image`-crate QF=85 recompression; step 256 gets 100% stability. The constant is NOT changed in this release (stego-breaking — would require envelope version bump); env knob lets future work iterate cleanly. (2) **`phantasm-cost` sidecar reader now preserves `f64::INFINITY`** (commit `9312ecc`): the v0.3 `is_finite() && > 0.0 → 1.0` sanitizer silently coerced wet-paper markers into cost = 1.0 (vs typical J-UW costs ~864), inverting wet-paper semantics into accidental cost-attraction. Replaced with explicit `sanitize_sidecar_cost()`; 3 new tests; 280 passing total. (3) **L1-defense research arc** (HYDRA / CHAMELEON / DOPPELGÄNGER / PALIMPSEST) ran on private `experiment/*` branches and is closed negatively across the cost-map family of defenses. Lesson: the detectable signature is the FACT of modification (decode JPEG → ±1 modify integer DCT coefficient → re-encode), not WHERE; future L1 work must change the embedding *operator* (OUROBOROS, GHOSTWRITER specs at `scratch/specs/2026-04-25-*.md`), not the position distribution. **No breaking changes** to the v0.3 envelope (still v2) or default behavior on existing flags.
